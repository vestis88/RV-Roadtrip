...
- [ ] **Map UI polish: marker size and selection highlight** (screenshot 2026-07-27) — both `DayViewScreen.tsx` and `OverviewMapScreen.tsx` render markers as bare emoji (`CATEGORY_ICON`/`RESTAURANT_ICON`/`OVERNIGHT_ICON`, `src/lib/mapIcons.ts`) inside `<AdvancedMarker>` with no size/background styling, so they render at default browser font-size — tiny and hard to tap, especially at the zoom levels real restaurant clusters show up at (see screenshot: several overlapping pins in Varberg's old town). Three fixes, all touching the same markers/cards so likely worth doing together:
  1. Wrap each marker's icon in a sized/padded container (e.g. a circular badge) instead of a bare `<span>`, in both screens.
  2. Day View already tracks `selectedPlace` (state driving both card highlighting and `MapPanner`) but only uses it to *pan* the map — the marker itself doesn't visually change when its card is selected (tapping a card doesn't highlight the matching pin, only the reverse works one-way visually). Give the marker matching `selectedPlace.id` a distinct look (scale up, outline, or color change) so the map and the highlighted card agree on which place is selected.
  3. **"Select" redesign (2026-07-27 report — "selecting things doesn't seem to do anything"; investigated same day).** Correction to the first read of this: `status: 'selected'` (set by the card's "Select" button — "this is actually in my plan", distinct from `selectedPlace`'s transient tap-to-view state above) is NOT entirely inert — `mapZoomTiers.ts`'s original Section 7.2 spec ("9–11: selected activities") is real and wired up in `OverviewMapScreen.tsx`: at zoom 9–11, only activities with `status === 'selected'` render as pins. But that's the *only* place it has any effect: it doesn't touch Day View at all (where the Select button actually lives — `PlaceCard.tsx` only changes a small gray "Status: selected" text label there, no border/color change), it's invisible unless you're on a *different* screen at a specific zoom band, and restaurants get no equivalent filtering at all (always all-hidden or all-shown, never gated by selection). That mismatch — click Select, look at the same screen you clicked it on, see nothing — is almost certainly the whole complaint. Beyond just fixing visibility, a more useful design (ranked by value):
     1. **Immediate feedback where you clicked it** (the real fix for the reported bug): a real visual state on the card for `status === 'selected'` distinct from the tap-to-view highlight, and Day View's own map (not just Overview Map) distinguishing selected pins from merely-suggested ones.
     2. **Turn the AI's shotgun of options into "our plan"**: 5 activities and 9 restaurants a day is a menu, not an itinerary. A toggle on Day View ("All suggestions" vs "Our picks") showing only selected items would make Select the actual curation step it's meant to be, instead of a side note buried under suggestions.
     3. **Extend the existing Overview Map zoom-tier filtering to restaurants**, consistent with activities, instead of today's all-or-nothing behavior.
     4. **Feed selections into replan**: a "Request changes" replan currently has no notion of what was already selected for a day being regenerated; a selected-but-not-done item could be treated as a soft preference to try to preserve, similar to how locked days already survive verbatim (lower priority than 1–3; also depends on replan's own Claude-call wiring, see T-14/T-17's notes on `runReplan` still being fixture-based).

- [ ] **Overnight-stop type & candidate selection** (requested 2026-07-27) — there is currently no way to choose *where* to sleep beyond whatever single point the AI picked. `overnightStopSchema` (`shared/src/schemas.ts`) is one committed point per day (`name, lat, lng, country`) plus an optional free-text `campsiteSuggestion` string — no structured type, and Claude/Places never propose alternatives the way they do for activities/restaurants (5 and 9 candidates respectively). Requested: let the traveler choose between camping types per night — commercial **campsite**, Nordic/European **"ställplats"**-style motorhome parking (simpler/cheaper, usually just services + a spot, no full campground amenities), and **nature/wild camping** suggestions. This is a bigger feature than it first looks:
  - Schema: `overnightStopSchema` needs a `type: 'campsite' | 'stellplatz' | 'wild'` field (or similar), and the day needs to carry a short list of candidate overnight stops instead of one committed point — same shape shift T-16's activities/restaurants already went through (propose several, let the user pick, only one ever gets used for the next day's drive-leg calculation).
  - Data source: Google Places (New) has decent commercial campsite coverage but weak-to-nonexistent structured data for "ställplats" specifically and none for wild-camping legality/suitability (that's inherently more of a "known spots + local rules" problem than a ratings/reviews problem) — may need Claude's own knowledge/reasoning for those two types rather than a Places lookup, with appropriately cautious phrasing (legality of wild camping varies a lot by country — this should probably lean on the same country-guide info T-18/6.3 already gathers, e.g. `freeCampingRules`).
  - UI: a candidate-picker similar in spirit to activity cards, surfaced on the day this stop applies to.
  Scope this as its own design pass rather than folding into the "manual editing" item above — picking an overnight type has real downstream effects (drive-leg endpoint, next day's starting point) that a manually-added activity doesn't.
