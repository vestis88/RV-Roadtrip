...
- [ ] **Trip-sharing UX** (surfaced answering "how would the trip be shared between devices" 2026-07-27) — the mechanism works (Section 4: 6-char `shareCode`, `joinTrip` callable adds the joining device's anonymous UID to `members`) but the UI around it is minimal: `AppShell.tsx` just displays "Share code: XXXXXX" as plain text (no copy button), and the *only* way to actually join is a second device visiting the app URL with `?join=CODE` appended (`useTripSession.ts`) — there's no in-app "Enter a share code" input field for a device that already has the app open. Small, cheap UI addition: a "Copy link" button (`${origin}?join=${shareCode}`) next to the share code, plus a manual code-entry field as a fallback to the URL-param path.
