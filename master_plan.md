...
- [ ] **Map UI polish: marker size and selection highlight** (screenshot 2026-07-27) — both `DayViewScreen.tsx` and `OverviewMapScreen.tsx` render markers as bare emoji (`CATEGORY_ICON`/`RESTAURANT_ICON`/`OVERNIGHT_ICON`, `src/lib/mapIcons.ts`) inside `<AdvancedMarker>` with no size/background styling, so they render at default browser font-size — tiny and hard to tap, especially at the zoom levels real restaurant clusters show up at (see screenshot: several overlapping pins in Varberg's old town). Three fixes, all touching the same markers/cards so likely worth doing together:
  ...
  3. **Confirmed separate issue, same root cause (2026-07-27 report — "selecting things doesn't seem to do anything"):** the persisted `status: 'selected'` (set by the card's "Select" button — "this is actually in my plan", distinct from `selectedPlace`'s transient tap-to-view state above) has almost no visible effect anywhere. In `PlaceCard.tsx` it only changes a small gray "Status: selected" text label (`{testId}-status`) — no border/color change on the card itself (that styling is reserved for the transient `selected` prop, i.e. tap-to-view, not this). On the map, markers never read `status` at all, so a day full of "selected" plans looks visually identical to one that's all untouched AI suggestions. Needs: a real visual state on the card for `status === 'selected'` (distinct from the tap-to-view highlight), and marker styling on the map driven by `status` (e.g. filled/starred marker for selected items vs. plain for merely-suggested ones) so the traveler can see their actual finalized plan at a glance.
