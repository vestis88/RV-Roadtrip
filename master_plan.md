...
- [ ] **Persistent, always-editable route corridor on the Map tab** (requested 2026-07-29; scoped 2026-07-29, not yet implemented — retires the highlights-review pause above once landed) — the highlights-review pause ("Interactive / transparent route planning" above) is a one-shot, ephemeral gate: `RegionHighlightsResponse` lives only in `planMeta.pendingHighlights` while `status === 'awaiting-highlights-review'` and is deleted the instant the traveler submits it — confirmed nothing persists the curated highlights, the route outline, or any backbone/corridor geometry past generation. Separately, every replan path (`runReplan`, and by extension "Request changes" and "Change overnight stop") is destroy-and-regenerate of the whole unlocked remainder via a fresh 3-phase `planTrip()` call — never a diff against what's already there; a locked day is merely protected from deletion, not fed to Claude as a routing constraint, and any freshly-generated day whose date collides with a locked one is silently dropped (`replanTrip.ts`'s own comment admits this limitation).
  ...
  1. **Map tab interaction parity** (low risk, no schema change) — port Day View's click-a-pin-highlights-a-card pattern (`selectedPlace`/`MapPanner`/`MarkerBadge`'s `highlighted` prop) onto `OverviewMapScreen.tsx`, which today has no card list and no marker click behavior beyond day badges navigating to Day View. Add `planMeta.status` branching there too — currently the only primary-tab screen with none (`SettingsScreen.tsx` already has this pattern).
  2. **Persist the corridor** (medium risk, backend-only, no UI change) — new `corridorStops` shared type/collection (`trips/{id}/corridorStops/{stopId}`, auto-generated IDs — stable identity the current positional-index `RegionHighlightCandidate` shape doesn't have) with `status: proposed|committed|locked` and `linkedDayIds`; materialized alongside day writes in `generatePlan.ts`/`replanTrip.ts`. Bundled with it: migrate `trips/{id}/days` from date-keyed doc IDs to auto-generated IDs (date stays a field) — the concrete blocker `insertRestDay.ts`'s own comment names ("no atomic rename") for any future date-shift operation; this migration is what makes phase 4 possible at all.
  3. **Corridor editing + "rescan an area"** (medium risk, additive-only, never touches `TripDay`s) — corridor becomes a live map layer editable from `idle` status onward (add/remove/reorder/lock a stop, direct Firestore writes, same philosophy as `markSelected`/`AddCustomStopForm`). Rescan is a new callable modeled directly on `enrichHighlights.ts` (radius filter via `haversineDistanceKm` instead of route-detour filter), writing only `proposed` stops — structurally incapable of touching `committed`/`locked` ones, so no merge algorithm is needed for this phase.
  4. **"Lock in the new route"** — the reconciliation engine, split in two (mirroring how the highlights-review pause itself was deliberately scoped down to a v1 the first time):
     - **4a. Reorder/date-shift only** — new `reconcileCorridor.ts`: if the set of committed stops is unchanged and only their order moved, day dates/indexes get remapped in sequence (no delete/recreate, thanks to phase 2's ID migration — activities/restaurants subcollections untouched), drive legs recomputed via the existing Routes API path, pacing re-validated before commit. Reviewed via a diff screen (explicitly modeled on the highlights-review pause's own review-and-confirm UX) before anything writes.
  ...
  **Carry forward from this session's HighlightsReviewPanel/DayView/overnight-candidates work (2026-07-28/29) rather than re-deriving any of this from scratch** — every one of these was learned the hard way (a real bug, a real "doesn't work on phone" report) on data shaped almost exactly like a corridor's:
  - **List-linked map for MANY items is HighlightsReviewPanel's current layout, not Day View's caption.** An `InfoWindow` popup showing full candidate detail on marker-tap was tried first and reported as too big to use on a phone (it covered the whole map). Replaced with: map sticky/pinned at the top of the scroll area, candidate list scrolling underneath it, marker-tap scrolls the list to that row and highlights it (`scrollToStop` + a ref map keyed by stop id + `scroll-mt-*` on each row matching the sticky map's height so a scrolled-to row doesn't land hidden under it), list-tap pans/zooms the map (`PanToSelected`). This is the right reference for the corridor screen (phase 1 and especially phase 3), which — unlike Day View's handful of day badges — will show a real, possibly-long list of stops across regions. Day View's simpler `selectedPlace`/`MapPanner`/single-line "Showing: X" caption is the right size only for phase 1's narrower ask (making existing day-pins clickable at all); don't carry its *popup-free-but-single-item* assumption into phase 3's denser list without re-checking it still fits.
  - **Give every list-of-editable-items row a stable ref for scroll-to, keyed by a real ID.** The scroll-to-row mechanism above only works because rows are keyed by something stable — reinforces phase 2's stable-ID decision for `corridorStops`; positional (region-index/stop-index) identity in the current `HighlightsReviewPanel`/`RegionHighlightCandidate` shape was a repeat source of bugs this session (a removal shifting indices under a still-open selection, requiring an unconditional `setSelectedStop(null)` on every removal as a blunt workaround) that a real ID sidesteps entirely.
