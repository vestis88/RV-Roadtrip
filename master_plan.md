...
- [ ] **Map UI polish: marker size and selection highlight** (screenshot 2026-07-27) — both `DayViewScreen.tsx` and `OverviewMapScreen.tsx` render markers as bare emoji (`CATEGORY_ICON`/`RESTAURANT_ICON`/`OVERNIGHT_ICON`, `src/lib/mapIcons.ts`) inside `<AdvancedMarker>` with no size/background styling, so they render at default browser font-size — tiny and hard to tap, especially at the zoom levels real restaurant clusters show up at (see screenshot: several overlapping pins in Varberg's old town). Two fixes, likely worth doing together since both touch the same markers:
